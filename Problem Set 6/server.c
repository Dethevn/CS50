...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        // validates request-line
            ...
            // pointer to beginning of request-target
            char *rt = (strchr(line, ' ')+1);
            ...
            // setup var to test for period in request line
            int per = 0;
            
            // test characters in request line
            for (char *p = rt; *p != ' '; p++)
            {
                // ensure no quotation  marks in request line
                if (*p == '\"')
                {
                    error(400);
                    continue;
            ...
            // test HTTP string to be exactly HTTP/1.1
             char *h_vers = (strchr(rt, ' ')+1);
             
             if (memcmp(h_vers, "HTTP/1.1", 8) != 0)
             {
                error(505);
                continue;
             }
         ...
         // extract query from request-target
            ...
            // initialize to "" as default with no query
            char query[] = "";
            
            // index to loop through query
            int zz = 0;
            
            // look for a query '?'
            for (char *i = rt; *i != ' '; i++)
            {
            // if found, assign query to query char array
                if (*i == '?')
                {
                    for (char *j = i+1; *j != ' '; j++)
                    {
        ...
        // concatenate root and absolute-path
            ...
            // copy line to absolute_path buffer
            for (char *i = rt; *i != '?' && *i != ' '; i++)
            {
                absolute_path[xx] = *i;
                xx++;  
        ...
        // ensure path is readable
            ...
            // declare stat struct to test mode
            // Ref for guidance: http://linux.die.net/man/2/stat
            ...
            // bitwise AND test for '1' bit in 4 place where 4 indicates readable by others
        ...
        // extract path's extension
            ...
            // define pointer to '.' in request line
            char *ext = strchr(rt, '.');
            
            // assign letters of line to extension
            while (*ext != '?' && *ext != ' ')
            {
                extension[i] = *ext;
                ext++;
            ...
            else
            {
                ...
                // respond to client, if errors encountered (negative values) continue to accept input
...
const char* lookup(const char* extension)
{
    // point to the string beginning with .
    char *ext = strrchr(extension, '.');
    
    // compare string beginning with period to extension scenarios
    if (strcmp(ext, ".css")==0)
        return "text/css";
